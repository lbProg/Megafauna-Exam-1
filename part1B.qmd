...
*What are the knowledge gaps for many marine megafauna, including white sharks?*
...
*Describe briefly the sampling carried out in this study. What is the data sample size? Was the TL data collected in the same way for all sharks? If not, is this an issue? Please justify why you think it is an issue or not (mono-syllabic answer won’t get a full grade).*
...
*What is a maturity ogive? Please illustrate one using R: write a function to draw a maturity ogive from the necessary inputs. Use the input $L_{50} = 350$ and plot the resulting function over the support 200-500 cm for these following values of input $\phi \in {0, 0.5, 1, 2, 5, 10}$. Use different colors to display the results on the same plots with a legend on the right. Are there any constraints on the inputs $\theta = \phi, L_{50}$ ?*
...
*The authors chose to use informative prior distributions for the parameters $\theta = \phi, L_{50}$ with different priors for males and females. Plot the density of these priors using R: use facet to have sex on rows and parameters in columns for this plot. (hint: prepare a data.frame with a long format and use facet_grid)*

For the normal distribution of $L_{50}$, we assumed that the parameters provided by the authors corresponded to the mean and standard deviation, since they specified that the second term was indeed the standard deviation and not $\tau$.

In contrast, for the lognormal distribution of $\phi$, we assumed that the authors had given the meanlog and $\tau$, because BUG/JAGS expects as input a mean and a $\tau$ rather than a mean and a standard deviation. Moreover, they provided the formula for converting between the standard deviation and $\tau$ just afterwards, which reinforced our conclusion that, for the lognormal distribution, they had supplied the BUG/JAGS input arguments.
...
*Describe the chosen specifications for the MCMC analysis (nb or iteration, burnin, thinning) and how the authors assessed parameters convergence and carried out model selection (if any). Are there any correlations in the posterior distribution of  $\theta = \phi, L_{50}$ for either males or females? From figure 6, would you say that the data were informative on these parameters compared to the initial priors?*
...
*From table 3, reproduce the curves displayed on Figure 7. Use dashed lines for the lower and upper bounds, a solid line for the posterior mean and different colours for males and females.*
...
*How do the authors justify their resorting to a Bayesian framework? Why is knowledge of a maturity ogive important for conservation and management?*
...
*Write the nimble code for the ogive maturity model describe in the paper. Use the same likelihood and priors than those described in the paper.*
